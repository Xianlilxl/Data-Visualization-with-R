            ...
            tabItem(tabName = "ans", 
                    fluidRow(
                        tabBox(title = "Statistiques par an", id = "tabset1", width = 12,
                               tabPanel("Parametres", 
                                        checkboxGroupInput(inputId = "discipline", 
                               ...
                               tabPanel("Statistiques des salaires", 
                                        fluidRow(
                                            splitLayout(cellWidths = c("50%", "50%"),
                                                        ...
                                                        plotOutput(outputId = "Salaire_regional", height="450px")
                                            )
                                        )
                               )
                        ))),
...
server <- function(input, output) {
    ...
    output$Part_femmes <- renderPlot({
        ...
        diplome.lp <- read.csv('fr-esr-insertion_professionnelle-lp.csv', header = T, sep = ';', na.strings = c('ns', 'nd'), fill=TRUE, encoding = "UTF-8")%>%filter(Domaine == 'Sciences, technologies et santé')
        diplome.DUT <- read.csv('fr-esr-insertion_professionnelle-dut_donnees_nationales.csv', header = T, sep = ';',na.strings = c('ns', 'nd'), fill=TRUE, encoding = "UTF-8")%>%filter(Domaine == 'Sciences, technologies et santé')
        diplome.master <- read.csv('fr-esr-insertion_professionnelle-master.csv', header = T, sep = ';', na.strings = c('ns', 'nd'), fill=TRUE, encoding = "UTF-8")%>%filter(domaine == 'Sciences, technologies et santé')
        #print(summary(diplome.master))
    
        part.femme.DUT <- data.frame(Annee = diplome.DUT$Année, Diplome = diplome.DUT$Diplôme, Part_femme = as.numeric(diplome.DUT$Part.des.femmes))%>%group_by(Annee, Diplome)%>%summarise(Part_femmes = median(Part_femme, na.rm = TRUE))
        part.femme.lp <- data.frame(Annee = diplome.lp$Annee, Diplome = diplome.lp$Diplôme, Part_femme = as.numeric(diplome.lp$X..femmes))%>%group_by(Annee, Diplome)%>%summarise(Part_femmes = median(Part_femme, na.rm = TRUE))
        part.femme.master <- data.frame(Annee = diplome.master$annee, Diplome = diplome.master$diplome, Part_femme = as.numeric(diplome.master$femmes))%>%group_by(Annee, Diplome)%>%summarise(Part_femmes = median(Part_femme, na.rm = TRUE))
        ...
        ggplot(part.femme.df, aes(x = Annee, y = Part_femmes, group = Diplome, color = Diplome)) + geom_line() + labs(x = "Annees", y = "Part des femmes")
    })
